...
const defaultContent = `
  ...
  <section class="mb-8">
    ...
    <p style="color: #4A4035;">
      お客様の個人情報は、<a href="/privacy" class="underline" style="color: #B8956A;">プライバシーポリシー</a>に基づき、適切に管理いたします。
    </p>
  </section>
`

export const renderTokushohoPage = (policy: Policy | null = null) => {
  // DBにコンテンツがある場合はそれを使用、なければデフォルト
  const hasCustomContent = policy?.content && policy.content !== '（内容準備中）' && policy.content.trim() !== ''
  const displayContent = hasCustomContent ? policy.content : defaultContent
  const lastUpdated = policy?.last_updated 
    ? new Date(policy.last_updated).toLocaleDateString('ja-JP', { year: 'numeric', month: 'long', day: 'numeric' })
    : '2025年12月25日'
      ...
      <div class="max-w-3xl mx-auto">
        ...
        <div class="text-center mb-10">
          <h1 class="text-3xl font-bold mb-4" style="color: #3D3428;">
            <i class="fas fa-store mr-3" style="color: #B8956A;"></i>
            特定商取引法に基づく表記
          </h1>
        ...
        <div class="bg-white rounded-2xl shadow-lg overflow-hidden" style="border: 1px solid #E8DCC8;">
          ...
          <div class="p-8 tokushoho-content">
            ${displayContent}
          </div>
