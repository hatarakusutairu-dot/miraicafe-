...
### 2026-03-27
- **個別相談予約のGoogleカレンダー自動登録**
  - 予約申請時に仮予約（黄色）としてカレンダーに自動登録
  - 承認後は確定（緑色）に変更
  - Service Account認証で全イベント取得可能
- **カレンダー空き時間の正確な反映**
  - 終日イベント・長時間イベント対応
  - すべての予定（透明/不透明問わず）を予約不可に
  - 予定の前後1時間をバッファとして予約不可（移動・準備時間）
- **環境変数追加**: `GOOGLE_SERVICE_ACCOUNT_KEY`
